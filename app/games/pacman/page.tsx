...
export default function PacmanGame() {
  ...
  useEffect(() => {
    ...
    function handleKeyDown(e: KeyboardEvent) {
      switch (e.key) {
        ...
        case "w":
          if (
            canMove(gameState.pacman.x, gameState.pacman.y - gameState.cellSize)
          ) {
        ...
        case "a":
          if (
            canMove(gameState.pacman.x - gameState.cellSize, gameState.pacman.y)
          ) {
  ...
  function initializeGame(canvas: HTMLCanvasElement) {
    ...
    gameState.pacman.x =
      Math.floor(gameState.gridWidth / 2) * gameState.cellSize;
    gameState.pacman.y =
      Math.floor(gameState.gridHeight / 2) * gameState.cellSize;
    ...
    gameState.walls = [
      ...
      {
        x: 0,
        y: canvas.height - gameState.cellSize,
        width: canvas.width,
        height: gameState.cellSize,
      },
      {
        x: canvas.width - gameState.cellSize,
        y: 0,
    ...
    gameState.dots = [];
    for (
      let x = gameState.cellSize;
      x < canvas.width - gameState.cellSize;
      x += gameState.cellSize
    ) {
      for (
        let y = gameState.cellSize;
        y < canvas.height - gameState.cellSize;
        y += gameState.cellSize
      ) {
  ...
  function canMove(x: number, y: number) {
    ...
    if (
      x < 0 ||
      y < 0 ||
      x + gameState.cellSize > gameState.gridWidth * gameState.cellSize ||
      y + gameState.cellSize > gameState.gridHeight * gameState.cellSize
    ) {
      return false;
    }
    ...
    for (const wall of gameState.walls) {
      if (
        x < wall.x + wall.width &&
        x + gameState.cellSize > wall.x &&
        y < wall.y + wall.height &&
        y + gameState.cellSize > wall.y
      ) {
        return false;
      }
    }
  ...
  function drawGame(ctx: CanvasRenderingContext2D) {
    ...
    gameState.dots.forEach((dot) => {
      ctx.beginPath();
      ctx.arc(
        dot.x + gameState.cellSize / 2,
        dot.y + gameState.cellSize / 2,
        3,
        0,
        Math.PI * 2
      );
    ...
    gameState.ghosts.forEach((ghost) => {
      ...
      // Ghost "skirt"
      ctx.lineTo(
        ghost.x + gameState.cellSize,
        ghost.y + gameState.cellSize / 2 + 6
      );
